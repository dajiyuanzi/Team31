...
<?php
  ...
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      ...
      echo "  <p><a href='../frontend/commentVisitor.php?tid=".$row['tid']."'>".htmlspecialchars($row["description"])."</a><br>Likes: <span id='liketid".$row['tid']."'>".$row["like"]."</span> Dislikes: <span id='disliketid".$row['tid']."'>".$row["dislike"]."</span> Popularity: <span id='popularityid".$row['tid']."'>$popularity</span>";
